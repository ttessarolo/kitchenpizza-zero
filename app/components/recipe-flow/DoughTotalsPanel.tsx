...
export function DoughTotalsPanel() {
  ...
  // Breakdown: from graph if available, otherwise estimate from portioning profile
  ...
  return (
    <div className="bg-gradient-to-br from-[#f9f3ec] to-[#f5ede3] rounded-[7px] p-2.5">
      ...
      <div className="flex justify-between items-center text-xs">
        <span className="text-muted-foreground font-medium">Totale impasto</span>
        <div className="flex items-center gap-1">
          <input
            ...
            step={10}
            min={50}
            onChange={(e) => scaleAllNodes(+e.target.value || 50)}
      ...
      <div className="flex items-center gap-1 text-xs text-muted-foreground mt-2">
        ...
        <input
          ...
          value={displayHyd}
          step={1}
          onChange={(e) => setGlobalHydration(+e.target.value || 0)}
      ...
      <div className="grid grid-cols-2 gap-x-3 gap-y-1 mt-2 text-xs">
        <div className="flex justify-between">
          ...
          <span className="font-bold text-foreground">{rnd(estFlour)}g</span>
        </div>
        <div className="flex justify-between">
          <span className="text-muted-foreground">Liquidi</span>
          <span className="font-bold text-foreground">{rnd(estLiquid)}g</span>
        </div>
        <div className="flex justify-between">
          <span className="text-muted-foreground">Lievito</span>
          <span className="font-bold text-foreground">{estYeast}g</span>
        </div>
        <div className="flex justify-between">
          <span className="text-muted-foreground">Sale</span>
          <span className="font-bold text-foreground">{estSalt}g</span>
        </div>
        <div className="flex justify-between">
          <span className="text-muted-foreground">Grassi</span>
          <span className="font-bold text-foreground">{estFat}g</span>
        </div>
      </div>
    </div>
  )
