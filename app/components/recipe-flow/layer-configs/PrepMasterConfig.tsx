...
const PREP_TYPES = ['generic', 'topping', 'filling', 'garnish', 'base', 'other'] as const
...
export function PrepMasterConfig() {
  ...
  return (
    <div className="space-y-3">
      <div>
        <label className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wider block mb-1">{t('prep_type')}</label>
        <select value={cfg.prepType} onChange={(e) => update({ prepType: e.target.value })}
          ...
          {PREP_TYPES.map((pt) => <option key={pt} value={pt}>{t(`prep_type_${pt}`)}</option>)}
        </select>
      </div>
      <div>
