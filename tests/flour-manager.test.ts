...
describe('FlourManager — getFloursByGroup', () => {
  it('returns only Grano Tenero flours', () => {
    const gt = getFloursByGroup('flour_group_grano_tenero', catalog)
  ...
  it('returns only Grano Duro flours', () => {
    const gd = getFloursByGroup('flour_group_grano_duro', catalog)
    expect(gd.length).toBe(3)
    expect(gd.every((f) => f.groupKey === 'flour_group_grano_duro')).toBe(true)
  })

  it('returns empty for unknown group', () => {
    expect(getFloursByGroup('Nonexistent', catalog)).toHaveLength(0)
  })
})

describe('FlourManager — searchFlours', () => {
  it('finds by labelKey', () => {
    const results = searchFlours('flour_gt_manit', catalog)
    expect(results.length).toBe(1)
    expect(results[0].key).toBe('gt_manit')
  })

  it('finds by subKey', () => {
    const results = searchFlours('flour_sp_sarac_sub', catalog)
    expect(results.length).toBeGreaterThanOrEqual(1)
  })

  it('case insensitive', () => {
    expect(searchFlours('FLOUR_GT_MANIT', catalog).length).toBe(1)
  })

  it('returns empty for no match', () => {
    expect(searchFlours('xyznonexistent', catalog)).toHaveLength(0)
  })
})
...
describe('FlourManager — estimateW', () => {
  it('estimates W for typical protein levels', () => {
    expect(estimateW(12, provider)).toBe(194)
    expect(estimateW(14, provider)).toBe(238)
  })
...
describe('FlourManager — isWholeGrain', () => {
  it('detects whole grain (fiber > 6)', () => {
    expect(isWholeGrain(getFlour('gt_int', catalog))).toBe(true) // fiber 9
    expect(isWholeGrain(getFlour('gt_00_for', catalog))).toBe(false) // fiber 2.2
  })
})

describe('FlourManager — isGlutenFree', () => {
  it('detects gluten-free (W=0, fermentSpeed=0)', () => {
    expect(isGlutenFree(getFlour('sp_sarac', catalog))).toBe(true)
    expect(isGlutenFree(getFlour('sp_riso', catalog))).toBe(true)
    expect(isGlutenFree(getFlour('gt_00_for', catalog))).toBe(false)
  })
})
